...
%output: a cell structure of the true 
          ...
          %"V"
%%%%%%%%%%%%%%%%%%%%
function cond_vals=true_cond(V, plot,Vertices,ll)
n_rings=size(V,2);%number of rings/radial discretization points 
                     %(excluding center)

 for i=1:n_rings
    cond_vals{i}=EvalConductivity(V{i},Vertices,ll);%construct the 
 ...
 if strcmp(plot,'yes')%if plot is yes
   ...
   cond_plot=EvalsmoothConductivity(Vert,Vertices,ll);%construct the conductivities 
